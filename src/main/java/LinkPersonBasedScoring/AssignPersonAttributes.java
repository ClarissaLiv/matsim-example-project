...
public class AssignPersonAttributes {
	...
	static public void main (Scenario scenario, Population population){
		...
		for (Person person : scenario.getPopulation().getPersons().values()) {
			...
			if (r<= 0.01) {
				...
				person.getAttributes().putAttribute(AGE_GROUP_DUMMY_2, 1);
			} else if (r<= 0.49) {
				person.getAttributes().putAttribute(AGE_GROUP_DUMMY_3, 1);
			} else if (r<= 0.79) {
				person.getAttributes().putAttribute(AGE_GROUP_DUMMY_4, 1);
			} else if (r<= 0.94) {
				person.getAttributes().putAttribute(AGE_GROUP_DUMMY_5, 1);
			} else {
				person.getAttributes().putAttribute(AGE_GROUP_DUMMY_6, 1);
			}
		}
